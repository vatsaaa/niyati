...
describe('useLogin', () => {
  ...
  it('calls processCompleteProfile when profile is complete', async () => {
    const profile = { name: 'T', birthDate: '1990-01-01', placeOfBirth: 'X', timeOfBirth: '00:00:00', consentGiven: true };
    ...
    expect(processCompleteProfile).toHaveBeenCalledWith(expect.objectContaining({ name: 'T' }), auth.countries, auth.phoneNumber);
  });
});
