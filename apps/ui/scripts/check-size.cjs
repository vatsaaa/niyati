...
// thresholds (tunable). Accepts raw bytes or human-friendly strings like '150KB', '2MB'
function parseLimit(envVal, defaultBytes) {
  if (!envVal) return defaultBytes
  const v = String(envVal).trim().toUpperCase()
  const kb = /^([0-9]+)K(B)?$/i
  const mb = /^([0-9]+)M(B)?$/i
  const num = /^([0-9]+)$/
  let m
  if ((m = v.match(mb))) return parseInt(m[1], 10) * 1024 * 1024
  if ((m = v.match(kb))) return parseInt(m[1], 10) * 1024
  if ((m = v.match(num))) return parseInt(m[1], 10)
  // Fallback parseInt
  const parsed = parseInt(envVal, 10)
  return isNaN(parsed) ? defaultBytes : parsed
}

const PER_FILE_LIMIT = parseLimit(process.env.PER_FILE_LIMIT, 150 * 1024)
const TOTAL_JS_LIMIT = parseLimit(process.env.TOTAL_JS_LIMIT, 250 * 1024)
