...
describe('bff-auth telemetry routes', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    jest.mock('@niyati/commons/lib/logger', () => ({ logger: { debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn(), fatal: jest.fn(), trace: jest.fn(), debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn() }, reqIdFromReq: () => null }));
    jest.mock('@niyati/commons/lib/sanitize', () => ({ sanitize: v => v }));
    jest.mock('@niyati/commons/config', () => ({ server: { apiVersion: '1.0' }, env: 'test' }));

    const createTelemetryRouter = require('@niyati/commons/lib/telemetry');
