...
## Running integration tests with Postgres (Docker)

You can run a throwaway Postgres instance locally and apply migrations before running integration tests.
...
2. Run migrations (script uses `DATABASE_URL` or defaults to `postgresql://postgres:postgres@localhost:5432/niyati_test`):
...
4. Tear down the database when finished:
...
Notes:
- The migration runner is `scripts/run_migrations.js` and will create a `migrations` table to track applied SQL files under `be/bff/migrations/`.
- In CI the same `npm run migrate:test` command is invoked after the Postgres service starts.
