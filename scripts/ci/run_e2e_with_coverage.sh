...
exit 0
#!/usr/bin/env bash
# Moved run_e2e_with_coverage.sh into scripts/ci
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ORIG="$(cd "$SCRIPT_DIR/.." && pwd)/run_e2e_with_coverage.sh"
if [[ -f "$ORIG" && "$ORIG" != "$0" ]]; then
  exec "$ORIG" "$@"
fi
echo "[ci/run_e2e_with_coverage] No original delegate found; nothing to run."
exit 0
